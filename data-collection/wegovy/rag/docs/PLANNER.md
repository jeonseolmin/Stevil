...
## Shared saved plans — local, not yet deployed

Calendar days now show an optional snack suggestion or availability explanation whenever an eligible protein gap exists. Selecting it opens that date's candidates; only adding a candidate and explicitly saving persists a SNACK event.

DietManagement and ExerciseManagement include SavedPlanPanel, reading the same authenticated user/week plan API. Diet shows MEAL + SNACK with macros; exercise shows EXERCISE with time/intensity/details. Date selection supports saved weeks. Completion checks save the full revision-protected plan and preserve all other event kinds; errors and 409 conflicts do not falsely mark completion. Successful saves notify mounted views; window focus reloads clean views, while unsaved dashboard edits are preserved with a reload warning. Actual intake/exercise log APIs are unchanged. No additional table or duplicated health records.

Verified frontend build and scoped lint, plus mocked browser calendar suggestion → add/save → shared food/snack view → completion in food/exercise view reflected in dashboard → 409 protection and reload, at 390px. Authenticated live DB verification was not performed.

## Optional protein snacks — local, not yet deployed

The selected day's planned nutrition is compared with the confirmed protein target. When all three meals have complete nutrition and the gap is at least 1g, the UI may offer one shake, chicken-breast and whole boiled-egg candidate that fits the remaining calorie allowance. No personal intake deficit is inferred from unchecked meals. Missing meal data, allergy/medical-limit text, an already-added snack or no confirmed goal suppress recommendations; no free inter-meal slot or insufficient calorie room produces an explanation. Normal food preferences are not model-screened for these optional product examples; users review product ingredients before choosing.

The official MFDS API supplied six catalog examples in `stevil-backend/src/main/resources/planner/snacks.json`, served under authenticated `GET /api/planner/snacks`. Each retains code, collection date, reference gram weight, original normalized nutrients, proposed amount, calculated nutrients and fingerprint. Proposed portions: shake product itself 30g (no added milk included), prepared chicken breast 50g, whole boiled egg edible portion 50g. These are selectable examples, not prescribed amounts, scoops or egg counts. Yolk/white-only records and ml-only nutrient references are excluded. Refresh with `python snack_catalog.py`; `--refresh` rebuilds from ignored `cache/nutrition/snack-source.json` without another download.

Recommendations do not affect totals until chosen. Selection adds a `SNACK` event in a conflict-free 10-minute interval between meals (30-minute meal margins, afternoon preferred), retains food evidence and marks the calendar dirty. At most one recommended snack per day is added. Explicit save persists it in the existing plan JSON. `SNACK` contributes to daily energy/macros and completed food-event counts; the UI switches the count unit to events when snacks exist. The API validates single-component `snack` evidence with the same nutrient scaling checks as composed meals. No database column change is needed.

Verified pure selection/availability/energy/missing-data cases, source egg classification and sodium formatting, Spring catalog evidence + SNACK JSON persistence, frontend build/lint, and browser recommendation → add → completion → mocked save/reload at 390px. Live authenticated database persistence was not tested. Restart the local Spring application to load `/api/planner/snacks` and the new event kind. A future server deployment must include the new classpath JSON resource as well as frontend/backend changes; the production deployment record below predates snacks.

## Production deployment — 2026-09-03

Deployed the local planner/profile/nutrition changes, grouped-number fix, error-stage handling, completion summary, diversified candidates and source component UI to `15.165.242.94`. Frontend, backend and RAG images were rebuilt and replaced. PostgreSQL was not replaced; `ddl-auto: update` and existing production configuration were preserved. Food and nutrient SQLite snapshots and completed embeddings were included in the RAG image (279 eligible additional foods). The additional collection API key stays local: generation reads the packaged snapshot and uses the existing production Gemini configuration.

Backup: `/home/ubuntu/stevil-rag/backups/nutrition-20260903T023627Z` contains prior app/RAG source and PostgreSQL dump. Rollback images: `stevil-frontend:before-20260903T023627Z`, `stevil-backend:before-20260903T023627Z`, `stevil-rag-api:before-20260903T023627Z`. Build/test logs and deployment scripts: `/home/ubuntu/stevil-rag/nutrition-deploy`.

Verified the built RAG image's 19 Python tests and complete nutrient index, nginx configuration, backend startup and actual production RAG generation: 21 meals, 15 compositions, 21 distinct titles. All component source fingerprints, normalized source nutrients, portion scaling, aggregate totals and exercise intervals matched. Anonymous user/planner/profile/draft requests returned 401. Public frontend serves `/assets/index-CEX9QvFz.js` with the new composition-count UI. Actual authenticated browser save/reload against production PostgreSQL was not exercised. Prior local-only deployment notes below are historical and superseded by this record.
