...
from planner.nutrition.matching import match_week, NutritionTargetUnavailable
from planner.nutrition.completion import complete_nutrition, load_snacks
from planner.service import schedule
from tests.test_planner import preferences
...
def composed(i, kcal):
    ...
    components = [
        dict(
            ...
            foodId=f'{i}-{role}',
            basisWeight='400',
            servingWeight=str(400 / 3),
            nutrition=nutrients,
            amountNutrition={k: str(float(v) / 3) for k, v in nutrients.items()},
            fingerprint='a' * 64,
        )
...
def snack(kcal=200):
    ...
    return dict(
        ...
        foodEvidence={
            'servingWeight': '400',
            'nutrition': {k: r[k] for k in ('INFO_ENG', 'INFO_PRO', 'INFO_CAR', 'INFO_FAT')},
        },
    )
...
class NutritionScheduleTest(unittest.TestCase):
    def setUp(self):
        ...
        validation = patch('planner.nutrition.completion.validate_food_policy')
        validation.start()
        self.addCleanup(validation.stop)

        self.p = preferences()
        self.p['nutritionGoal'] = dict(
            weightKg=70,
            proteinPerKg=1,
            calories=3000,
            confirmed=True,
    ...
    def plan(self, kcal, snacks=()):
        days, _ = match_week(
            [composed(i, kcal) for i in range(3)],
            self.p['nutritionGoal'],
            snacks,
        )
        suggestions = [
            dict(
                meals=[
                    dict(title='식사', details='제안량', foodEvidence=r['_evidence'])
                    for r in day
                ],
                exercise=dict(title='운동', details=''),
                plannedSnacks=day[0]['_plannedSnacks'],
            )
            for day in days
        ]
    ...
    def test_2100_is_adjusted_with_practical_amounts_without_snacks(self):
        ...
        monday = [
            ...
            if e['start'][:10] == self.p['weekStart'] and e['kind'] == 'MEAL'
        ]
        ...
        for e in monday:
            for c in e['foodEvidence']['components']:
                ...
                self.assertAlmostEqual(
                    float(c['amountNutrition']['INFO_ENG']),
                    float(c['nutrition']['INFO_ENG']) * float(c['servingWeight']) / 400,
                    places=3,
                )
    ...
    def test_impossible_target_and_missing_meal_fail(self):
        with self.assertRaises(NutritionTargetUnavailable):
            match_week(
                [row(i, 700, 25) for i in range(3)],
                self.p['nutritionGoal'],
            )

        suggestions, result = self.plan(
            700,
        ...
        with self.assertRaises(NutritionTargetUnavailable):
            complete_nutrition(self.p, result, suggestions)

    def test_work_allows_automatic_snack(self):
        self.p['busySlots'] = [
            dict(day=0, start='09:00', end='17:00', allowMeals=True, allowSnacks=True)
        ]
        suggestions, result = self.plan(600, [snack()])
        ...
        self.assertTrue(any(
            e['kind'] == 'SNACK'
            and '2026-09-07T09:00' <= e['start'] < '2026-09-07T17:00'
            for e in events
        ))
    ...
    def test_original_rows_unchanged_and_catalog_shared(self):
        rows = [composed(i, 700) for i in range(3)]
        ...
        self.assertEqual(
            load_snacks(),
            json.loads(backend.read_text(encoding='utf-8')),
        )

    def test_target_boundaries_and_two_snacks(self):
        for kcal in (570, 630):
            days, _ = match_week(
                [row(i, kcal, 25) for i in range(3)],
                self.p['nutritionGoal'] | {'calories': 1800},
            )
            self.assertEqual(len(days), 7)

        suggestions, result = self.plan(
            600,
            [snack(100), dict(snack(110), id='second')],
        )
        result = complete_nutrition(self.p, result, suggestions)
    ...
    def test_2500_goal_does_not_force_exact_calories(self):
        ...
        total = sum(
            float(e['foodEvidence']['nutrition']['INFO_ENG'])
    ...
    def test_1200_meal_cannot_pass_with_balanced_daily_total(self):
        ...
        with self.assertRaises(NutritionTargetUnavailable):
            match_week(
                [row(1, 650, 25), row(2, 650, 25), row(3, 1200, 25)],
                goal,
            )

        days, _ = match_week(
            [composed(1, 650), composed(2, 650), composed(3, 1200)],
            goal,
            [snack(), dict(snack(), id='second')],
        )
    ...
    def test_snacks_preferred_to_inflating_all_meals_and_spread_out(self):
        ...
        monday = [
            e for e in result['events']
            if e['start'][:10] == self.p['weekStart']
        ]
        ...
        self.assertTrue(
            all(float(e['foodEvidence']['nutrition']['INFO_ENG']) <= 900 for e in meals)
        )
