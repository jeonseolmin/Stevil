...
from planner.service import schedule, validate_preferences, minutes
...
def suggestions():
    return [
        dict(
            meals=[dict(title=t, details='예시') for t in ['아침', '점심', '저녁']],
            exercise=dict(title='활동', details='예시'),
        )
        for _ in range(7)
    ]
...
class PlannerTest(unittest.TestCase):
    def test_work_allows_meals_but_not_exercise_or_other_fixed_slots(self):
        ...
        self.assertTrue(all(
            e['start'][11:] >= '17:00' or e['end'][11:] <= '09:00'
            for e in events if e['kind'] == 'EXERCISE'
        ))
    ...
    def test_busy_time_moves_meal_without_overlap(self):
        p = preferences()
        p['busySlots'] = [dict(day=0, start='12:00', end='14:00')]
    ...
    def test_full_day_has_no_scheduled_events(self):
        p = preferences()
        p['busySlots'] = [dict(day=0, start='00:00', end='23:59')]
        result = schedule(p, suggestions())
    ...
    def test_overnight_and_invalid_week_rejected(self):
        ...
        p = preferences()
        p['weekStart'] = '2026-09-08'
        with self.assertRaises(ValueError):
            validate_preferences(p)

    def test_java_time_format(self):
        self.assertEqual(minutes('07:00:00'), 420)

    def test_exercise_stays_inside_day_window(self):
        p = preferences()
        p['exerciseWindows'] = [dict(day=0, start='09:00', end='10:00')]

        workouts = [
            e for e in schedule(p, suggestions())['events']
            if e['kind'] == 'EXERCISE' and e['start'].startswith('2026-09-07')
        ]
        ...
        self.assertFalse(any(
            e['kind'] == 'EXERCISE' and e['start'].startswith('2026-09-07')
    ...
    def test_short_and_duplicate_windows_rejected(self):
        p = preferences()
        p['exerciseWindows'] = [dict(day=0, start='09:00', end='09:05')]
        with self.assertRaises(ValueError):
            validate_preferences(p)
