...
from planner.food.nutrient_catalog import (
    normalize, grams, parse_page, initialize, ingest,
    NutrientCatalog, diverse_foods, nutrient_text,
)
...
ROWS = [
    fixture('rice', '현미밥', '밥류', 150, 32, 3, 1),
...
class NutrientCatalogTest(unittest.TestCase):
    ...
    def test_shortlist_deduplicates_variants_and_includes_cooking_categories(self):
        rows = [
            {'id': str(i), 'name': name, 'category': category}
    ...
    def test_units_and_missing_values_not_imputed(self):
        ...
        self.assertIsNone(normalize(ROWS[2] | {
            'FOOD_NM_KR': '오징어무침_채소',
            'FOOD_CAT1_NM': '생채·무침류',
        })[0])
    ...
    def test_response_shapes_and_auth_failure(self):
        ...
        for items in [ROWS, {'item': ROWS}]:
            self.assertEqual(
                parse_page({'header': header, 'body': {'items': items, 'totalCount': '3'}}),
                (ROWS, 3),
            )

        self.assertEqual(
            parse_page({'response': {'header': header, 'body': {'items': {'item': ROWS[0]}, 'totalCount': 1}}}),
            ([ROWS[0]], 1),
        )

        with self.assertRaises(RuntimeError):
            parse_page({'header': {'resultCode': '30'}})
    ...
    def test_sqlite_roundtrip_portions_sources_and_missing_sodium(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('planner.food.nutrient_catalog.VectorIndex') as index:
                ...
                self.assertEqual(
                    [c['servingWeight'] for c in ordinary[0]['_evidence']['components']],
                    ['200', '100', '80'],
                )
