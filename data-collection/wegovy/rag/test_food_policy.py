...
from food_policy import processed_meat, main_food_keys, validate_food_policy, FoodPolicyUnavailable
from nutrition import match_week
from nutrition_catalog import NutrientCatalog
...
class FoodPolicyTest(unittest.TestCase):
    ...
    def test_rice_and_portion_changes_do_not_hide_same_protein(self):
        ...
        events=[{'kind':'MEAL','title':'밥 '+str(i),'start':f'2026-09-{7+i:02d}T12:00','foodEvidence':a['_evidence']} for i in range(3)]
        with self.assertRaises(FoodPolicyUnavailable): validate_food_policy(events)

    def test_week_rotates_actual_ingredients_and_preserves_nutrition(self):
        names=['닭가슴살구이','돼지고기찜','소고기구이','오리구이','두부부침','계란찜',
        ...
        dishes=Counter();groups=Counter();events=[]
        for day,meals in enumerate(days):
            daily=Counter()
            for meal in meals:
                d,g=main_food_keys(meal);dishes.update(d);groups.update(g);daily.update(g)
    ...
    def test_existing_nutrient_index_is_reused_and_processed_food_filtered(self):
        from test_nutrition_catalog import ROWS, fixture, normalize
        catalog=NutrientCatalog.__new__(NutrientCatalog)
        catalog.metadata={'retrievedAt':'2026-09-03'}
        source=ROWS+[fixture('ham','런천미트구이','구이류',200,0,20,13)]
        catalog.rows={r['FOOD_CD']:normalize(r)[0] for r in source}
        with patch('nutrition_catalog.VectorIndex') as index:
            catalog.index=index.return_value
            catalog.index.ready=True
            catalog.index.rank.return_value=[('ham',1),('rice',.9),('fish',.8),('veg',.7)]
            meals=catalog.retrieve_meals({})
            self.assertTrue(meals)
            self.assertFalse(any(processed_meat(m) for m in meals))
