...
## 37. 2026-09-17 업데이트 — 병원찾기 DB 데모 데이터 확장 (팔달구 일대, 비율 배분)

§35~36 후속, DB만. 코드 변경 없음.

### 37.1 `수병원`을 제휴+광고로 전환

사용자가 "수병원을 제휴에 광고까지 달린 걸로" 요청. 네이버 API 직접 호출로 실존 확인(`수병원 | 정형외과 | 경기도 수원시 팔달구 경수대로 613 수병원`, 로컬 dev DB에 이미 있던 것과 동일 주소). 의사 계정이 1개(id=8)뿐이라 광고는 병원 하나에만 걸 수 있어, **기존에 광고가 걸려 있던 `우리의내과의원`에서 광고를 "옮기는"** 방식으로 처리:

```sql
UPDATE users SET nickname = '수병원' WHERE id = 8;                 -- 이전: '우리의내과의원'
INSERT INTO medical_facilities (...) VALUES ('HOSPITAL', '수병원', '경기도 수원시 팔달구 경수대로 613 수병원', ..., 'APPROVED', ...);  -- id=3
```

결과: `우리의내과의원`은 제휴만 남고(광고 소실), `수병원`이 제휴+광고. Rollback: `UPDATE users SET nickname='우리의내과의원' WHERE id=8; DELETE FROM medical_facilities WHERE id=3;`

### 37.2 팔달구 일대 비율 배분

"팔달구 일대를 뒤져서 일정 비율로" 요청. 네이버 지역검색을 7개 쿼리(병원/정형외과/이비인후과/피부과/소아과/안과/치과)로 넓게 돌려 실존 병원 30여 곳 확보 후, 그중 서로 다른 진료과 6곳을 추가로 `APPROVED` 제휴 처리(전부 실제 주소 확인 후 INSERT, 이름만으로 넣지 않음):

```sql
INSERT INTO medical_facilities (...) VALUES
  ('HOSPITAL','연세튼튼마취통증의학과의원','경기도 수원시 팔달구 중부대로 44 7층', ...),   -- id=4
  ('HOSPITAL','수원베스트이비인후과의원','경기도 수원시 팔달구 중부대로 44', ...),          -- id=5
  ('HOSPITAL','스노우의원 수원점','경기도 수원시 팔달구 효원로 279 3층', ...),              -- id=6
  ('HOSPITAL','수원아이편한병원','경기도 수원시 팔달구 권광로 138 수원시청역 선경센트럴파크 4층', ...), -- id=7
  ('HOSPITAL','삼성굿아이안과의원','경기도 수원시 팔달구 중부대로 44 건우까사미타워 601~605호', ...),   -- id=8
  ('HOSPITAL','웃는얼굴치과의원 수원점','경기도 수원시 팔달구 권광로 199 5층 503호', ...);   -- id=9
```

Rollback: `DELETE FROM medical_facilities WHERE id IN (4,5,6,7,8,9);`

### 37.3 최종 비율 (`medical_facilities` 9행 + 검색 풀 내 나머지는 전부 일반)

| 병원 | 상태 |
|---|---|
| 수병원 | 제휴 + 광고 |
| 권혁호내과의원, 우리의내과의원, 연세튼튼마취통증의학과의원, 수원베스트이비인후과의원, 스노우의원 수원점, 수원아이편한병원, 삼성굿아이안과의원, 웃는얼굴치과의원 수원점 | 제휴만 (8곳) |
| 가톨릭대학교 성빈센트병원 등 검색 풀의 나머지 20여 곳 | 일반(DB에 아무것도 안 넣음, 자동으로 일반) |

의사 계정이 1개뿐이라 광고는 여전히 `수병원` 한 곳에만 붙는다 — 여러 병원에 동시에 광고를 붙이려면 새 의사 계정(들)이 필요하며, 이번엔 만들지 않았다(§35에서 "기존 계정 재사용"으로 정한 방향 유지, 사용자가 새 계정 생성을 원하면 별도 확인 필요).

### 37.4 검증

브라우저의 기존 로그인 세션이 이번엔 만료돼(새 Browser pane 인스턴스, token 없음) 실시간 화면 확인은 못 함 — SQL SELECT로 최종 상태만 재확인. §35/§36에서 이미 동일 메커니즘(네이버 API 실주소 검증 → INSERT → SELECT 재확인)으로 실제 화면 렌더링까지 확인된 방식을 그대로 반복 적용한 것이라 신뢰도는 높음. 다음에 로그인 세션이 있을 때 "수원 팔달구"로 검색해 비율 육안 확인 권장.
