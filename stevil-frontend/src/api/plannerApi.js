...
function normalize(plan) {
    ...
    return { ...plan, preferences, events: plan.events.map(event => ({ ...event, start: event.start.slice(0, 16), end: event.end.slice(0, 16) })) };
}
export const loadWeek = (week) => axiosInstance.get("/planner", { params: { week } }).then(r => r.status === 204 ? null : normalize(r.data));
export const generateWeek = (preferences) => axiosInstance.post("/planner/draft", preferences, { timeout: 100000 }).then(r => r.data);
