    ...
    sources.forEach((source, sourceIndex) => {
        ...
        if (groupIndex === undefined) {
            ...
            groups.push({
                ...
                latest_version_verified: source.latest_version_verified,
                formulation: source.formulation,
                formulation_verified: source.formulation_verified,
        ...
        citationTargets[sourceIndex] = groupIndex;
    });
...
function uniqueSections(entries = []) {
    const seen = new Set();
    const sections = [];
...
function ChatTurn({ message, index }) {
    ...
    return (
        <article className="wegovy-chat__turn">
            <div className="wegovy-chat__question">
            ...
            <div className="wegovy-chat__reply">
                ...
                {message.notice && (
                    <p className="wegovy-chat__notice">
                        {message.notice}
                    </p>
                )}
...
export default function WegovyChatPage({
    ...
    async function submit(event) {
        ...
        try {
            const response = designPreview
                ? {
                    answer:
                        "궁금한 점을 편하게 남겨 주세요. 실제 서비스에서는 관련 자료를 찾아 답변과 출처를 함께 안내해 드려요.\n\n이 답변은 대화창의 모양을 확인하기 위한 디자인 예시입니다.",
                    sources: [],
                    mode: "design_preview",
                    notice:
                        "디자인 미리보기 · 실제 검색이나 API 요청은 전송하지 않았습니다.",
                }
                : await askRag(text);
            ...
            setError(
                err.name === "TimeoutError"
                    ? "답변을 받는 데 시간이 걸리고 있어요. 잠시 후 다시 보내 주세요."
                    : err.message
            );
        } finally {
            busy.current = false;
    ...
    return (
        <section
            className={`wegovy-chat${
                embedded
                    ? " wegovy-chat--embedded"
                    : ""
            }`}
        >
            <div className="wegovy-chat__container">
                {!embedded && (
                    <header className="wegovy-chat__heading">
                        <span className="wegovy-chat__eyebrow">
                            STEVIL GUIDE
                        </span>
                ...
                <div className="wegovy-chat__layout">
                    <div className="wegovy-chat__panel">
                        <div className="wegovy-chat__toolbar">
                            <div className="wegovy-chat__identity">
                                <span className="wegovy-chat__avatar">
                                    <Mark />
                                </span>
                        ...
                        <div
                            className="wegovy-chat__messages"
                            ref={conversation}
                            role="log"
                            aria-label="위고비 질문과 답변"
                            aria-live="polite"
                            tabIndex={0}
                        >
                            {!messages.length &&
                                !loading && (
                                    <div className="wegovy-chat__welcome">
                                        <span className="wegovy-chat__welcome-icon">
                                            <Mark book />
                                        </span>
                                        ...
                                        <div className="wegovy-chat__examples">
                                            {EXAMPLES.map(
                                                ([
                                                     label,
                                                     text,
                            ...
                            {loading && (
                                <div className="wegovy-chat__turn">
                                    <div className="wegovy-chat__question">
                                        {pending}
                                    </div>

                                    <p
                                        className="wegovy-chat__loading"
                                        role="status"
                                    >
                        ...
                        <div className="wegovy-chat__composer">
                            ...
                            <form onSubmit={submit}>
                                <label
                                    className="wegovy-chat__sr"
                                    htmlFor="wegovy-question"
                                >
                                    위고비 질문
                                </label>
                                ...
                                <textarea
                                    ...
                                    required
                                    disabled={loading}
                                    placeholder="위고비에 대해 궁금한 점을 입력해 주세요"
                                />

                                <div className="wegovy-chat__send-row">
                                    <span>
                                        {question.length} /
                                        1,000
                                    </span>
                    ...
                    {!embedded && (
                        <aside className="wegovy-chat__aside">
                            <section className="wegovy-chat__guide">
                                <span className="wegovy-chat__guide-icon">
                                    <Mark book />
                                </span>
                            ...
                            <section className="wegovy-chat__care">
                                <h2>
                                    이용 전 확인해 주세요
                                </h2>
                                ...
                                {status?.preview && (
                                    <span className="wegovy-chat__preview">
                                        검토 전 자료를 사용하는
                                        미리보기
                                    </span>
                                )}
