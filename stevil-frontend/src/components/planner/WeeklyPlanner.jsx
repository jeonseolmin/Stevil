...
import { DAYS, defaults, exampleWeek, monday, shiftDate, validatePlan, mealCalories, dayNutrition, calorieTargetStatus, dateKey, estimateCalories, defaultExerciseWindow } from "./plannerUtils";
...
function formatNutrient(value) {
    ...
    return Number.isFinite(calories) && calories >= 0
        ? calories.toLocaleString('ko-KR', { maximumFractionDigits: 1 }) : '—';
}

function Macros({ evidence, compact = false, rounded = false }) {
    const labels = [["INFO_CAR", "탄수화물", "탄"], ["INFO_PRO", "단백질", "단"], ["INFO_FAT", "지방", "지"]];
    return <span className={compact ? "planner-macros planner-macros--compact" : "planner-macros"} aria-label="탄수화물 단백질 지방 원문 수치">{labels.map(([key, label, short]) => <span key={key}><span>{compact ? short : label}</span><b>{rounded ? formatNutrient(evidence?.nutrition?.[key]) : evidence?.nutrition?.[key]?.trim() || "—"}</b></span>)}</span>;
}
...
function MealComponents({ components }) {
    return <div className="planner-meal-components" aria-label="한 끼 구성 음식">{components.map(food => <details key={food.foodId}>
        ...
        <p>제안량 기준 {food.role === "snack" ? formatNutrient(food.amountNutrition.INFO_ENG) : Number(food.amountNutrition.INFO_ENG).toFixed(0)} kcal · 단백질 {Number(food.amountNutrition.INFO_PRO).toFixed(1)}g</p>
        <Macros evidence={{ nutrition: food.amountNutrition }} compact rounded={food.role === "snack"} />
...
export default function WeeklyPlanner({ preview = false }) {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            finally { if (active && !operation.current) setBusy(""); }
        }
    ...
    const edit = (id, change) => { setEvents(items => items.map(item => item.id === id ? updatePlannerEvent(item, change) : item)); setDirty(true); setStatus(""); };
    ...
    async function generate(event) {
        ...
        if (operation.current || busy || !loaded) return;
        const invalid = validatePlan(preferences, []);
        if (invalid) return setError(invalid);
        // Lock before opening confirm: returning focus must not reload the saved plan.
        operation.current = true;
        if (events.length && !window.confirm("현재 주의 식사·간식·운동과 완료 체크를 새 초안으로 교체할까요? 저장된 일정은 확정 저장 전까지 유지됩니다.")) { operation.current = false; return; }
        setBusy("generate"); setError(""); setStatus("새 계획을 생성하고 있어요. 완료 전까지 기존 계획이 표시됩니다."); setNotices([]); setSelected(null);
        try {
            ...
            setStatus(demo ? "샘플 계획이에요. 일정을 눌러 수정해 보세요." : `식사 ${meals.length}끼 중 밥·반찬·채소 조합 ${composed}끼, 레시피 ${meals.length-composed}끼로 만들었어요. 새 초안으로 교체했으며 완료 체크는 초기화됐어요. 확정 저장하면 반영됩니다.`);
        } catch (err) { setStatus(events.length ? "재생성에 실패해 기존 계획을 유지했습니다." : ""); setError(err.response?.data?.message || err.message || "계획을 생성하지 못했어요."); }
    ...
    const targetStatus = calorieTargetStatus(preferences, todayEvents);
    ...
    function addSnack(candidate) {
        if (!snackOffer?.start || busy || todayEvents.filter(item => item.kind === "SNACK").length >= 2) return;
        ...
        if (invalid) return setError(invalid);
        setEvents(items => items.filter(item => item.kind === "SNACK" && item.start.slice(0,10) === summaryDate).length >= 2 ? items : [...items,event].sort((a,b)=>a.start.localeCompare(b.start)));
    ...
    return <section className="weekly-planner" aria-labelledby="planner-title" aria-busy={!!busy}>
        ...
        {targetStatus && <p role="status" className={`planner-target-status planner-target-status--${targetStatus.state}`}>{targetStatus.text} · 계획량 기준이며 실제 섭취량은 아닙니다.</p>}
        {goal?.confirmed && <div className="planner-week-targets" aria-label="요일별 목표 열량 충족 상태">{DAYS.map((day,index) => { const date = shiftDate(preferences.weekStart,index); const state = calorieTargetStatus(preferences,events.filter(e=>e.start.slice(0,10)===date)); return <button key={date} type="button" onClick={()=>setSummaryDate(date)} className={`planner-target-status--${state.state}`} aria-pressed={summaryDate===date}>{day} · {({macro_unbalanced:"탄단지 조정",unbalanced:"끼니 불균형",within:"충족",low:"부족",high:"초과",missing:"식사 누락",unknown:"확인 불가"})[state.state]}</button>; })}</div>}
        {goal?.confirmed && <p className="planner-help">생성 기준: 하루 탄수화물 45~65% · 지방 20~35% · 입력한 단백질 목표 ±20%. 비율은 탄단지 환산 열량 기준이며 일반 성인용 참고 범위입니다. <a href="https://www.ncbi.nlm.nih.gov/books/NBK208874/" target="_blank" rel="noreferrer">비율 참고 기준</a></p>}
        {snackOffer && <section ref={snackPanel} className="planner-snack-offer" aria-label="선택 가능한 단백질 간식">
            <header><strong>가볍게 보충하고 싶다면</strong><span>계획상 열량 {snackOffer.calorieGap} kcal · 단백질 {snackOffer.gap.toFixed(1)}g 부족</span></header>
            ...
            {snackError ? <p>간식 자료를 불러오지 못했어요. 화면을 새로고침해 주세요.</p> : <>
                ...
                <div className="planner-snack-options">{snackOffer.candidates.map(item=><article key={item.id}>
                    ...
                    <span>{item.foodEvidence.servingWeight}g · 단백질 {Number(item.foodEvidence.nutrition.INFO_PRO).toFixed(1)}g · {formatNutrient(item.foodEvidence.nutrition.INFO_ENG)} kcal</span>
        ...
        {formOpen && <form id="planner-settings" onSubmit={generate} className="planner-settings"><fieldset disabled={!!busy || !loaded}><legend>01 · 생활 리듬</legend><div className="planner-form-grid">{[["wakeTime", "기상"], ["sleepTime", "취침"], ["breakfastTime", "아침 식사"], ["lunchTime", "점심 식사"], ["dinnerTime", "저녁 식사"]].map(([key, label]) => <label key={key}>{label}<input type="time" required value={preferences[key]} onChange={event => update(key, event.target.value)} /></label>)}</div><p className="planner-help">현재는 같은 날 기상·취침하는 일정을 지원해요. 시간은 한국 시간 기준입니다.</p></fieldset>
            ...
            <fieldset disabled={!!busy}><legend>03 · 이미 정해진 일정</legend><p className="planner-help">업무·수업 시간을 등록하세요. 그 시간에 식사나 간식이 가능하면 각각 체크해 주세요. 위에서 설정한 식사 시간을 우선 사용하며 운동은 겹치지 않게 배치합니다.</p>{preferences.busySlots.map((slot, index) => <div className="planner-busy-row" key={index}><select aria-label={`고정 일정 ${index + 1} 요일`} value={slot.day} onChange={event => update("busySlots", preferences.busySlots.map((s, i) => i === index ? { ...s, day: Number(event.target.value) } : s))}>{DAYS.map((day, d) => <option value={d} key={day}>{day}요일</option>)}</select><input aria-label={`고정 일정 ${index + 1} 제목`} required maxLength={60} value={slot.title} onChange={event => update("busySlots", preferences.busySlots.map((s, i) => i === index ? { ...s, title: event.target.value } : s))} />{["start", "end"].map(key => <input key={key} aria-label={`고정 일정 ${index + 1} ${key === "start" ? "시작" : "종료"}`} type="time" required value={slot[key]} onChange={event => update("busySlots", preferences.busySlots.map((s, i) => i === index ? { ...s, [key]: event.target.value } : s))} />)}{[["allowMeals", "식사 가능"], ["allowSnacks", "간식 가능"]].map(([key, label]) => <label key={key} className="planner-busy-allow"><input type="checkbox" checked={slot[key] === true} onChange={event => update("busySlots", preferences.busySlots.map((s, i) => i === index ? { ...s, [key]: event.target.checked } : s))} />{label}</label>)}<button type="button" aria-label={`고정 일정 ${index + 1} 삭제`} onClick={() => update("busySlots", preferences.busySlots.filter((_, i) => i !== index))}>×</button></div>)}<button type="button" className="planner-secondary" disabled={preferences.busySlots.length >= 35} onClick={() => update("busySlots", [...preferences.busySlots, { day: 0, title: "업무", start: "09:00", end: "17:00", allowMeals: false, allowSnacks: false }])}>＋ 고정 일정 추가</button></fieldset>
        ...
        {chosen && <section className="planner-editor planner-detail" ref={editor} aria-label="일정 상세">
            ...
            {chosen.kind !== "EXERCISE" ? <>
                <div className="planner-detail-nutrients"><div className="planner-energy"><span>열량 · {chosen.foodEvidence?.components?.length ? "제안량 합계" : "원문"}</span><strong>{chosen.kind === "SNACK" ? formatNutrient(chosen.foodEvidence?.nutrition?.INFO_ENG) : chosen.foodEvidence?.nutrition?.INFO_ENG?.trim() || "—"}<small> kcal</small></strong></div><Macros evidence={chosen.foodEvidence} rounded={chosen.kind === "SNACK"} /></div>
                {chosen.foodEvidence ? <>
