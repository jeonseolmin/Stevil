import { dayNutrition } from './plannerUtils.js';
...
export function snackRecommendations(preferences, date, events, catalog) {
    ...
    if(meals.length!==3 || totals.partial || !totals.available || dayEvents.some(e=>e.kind==='SNACK')) return null;
    const gap=Math.round((goal.weightKg*goal.proteinPerKg-totals.protein)*10)/10;
    ...
    const occupied=[...dayEvents.map(e=>[minutes(e.start.slice(11)),minutes(e.end.slice(11))]),
        ...preferences.busySlots.filter(s=>s.day===weekday).map(s=>[minutes(s.start),minutes(s.end)])];
    let start=null;
    ...
    if(start===undefined || start===null) return {gap,reason:'식사 사이에 여유 시간이 없어 간식을 배치하지 않았어요.',candidates:[]};
    const types=new Set();
    ...
    return {gap,start:`${date}T${time(start)}`,end:`${date}T${time(start+10)}`,candidates,
